...
| 3 | 2026-09-03 | uart_rx_test | back-to-back (fork drive_rx_pin + clear_irq_process song song, item_done() ngay sau stop bit, không chờ gap) | 20 | 20 | 0 | p_data: giống hệt run #2, vẫn thiếu zero/alt2 (cùng seed random mặc định). cp_rx_gap: 33.33% (1/3 bin — back_to_back trúng cả 20/20 frame, đúng bản chất sequence này) — con số đã qua 2 lần sửa: lần đầu bug field sai class → không đổi (33.33% giả, do luôn =0 mặc định); sửa field đúng nhưng đo raw $time diff không trừ FRAME_CYCLES → 66.67% SAI (19 frame bị tính nhầm vào large_gap dù thực chất gap=0); sửa trừ FRAME_CYCLES đúng → 33.33% ĐÚNG, phản ánh trung thực rằng sequence back-to-back chỉ tạo đúng 1 kịch bản gap (=0), chưa hề chạm small_gap/large_gap | **33.33%** | Pass sạch 0 lỗi cả 3 lần. Bài học quan trọng: coverage % giảm không phải luôn là tin xấu — 66.67%→33.33% ở đây là sửa từ số liệu SAI (cao giả) sang số liệu ĐÚNG (thấp nhưng trung thực). Cần viết thêm sequence riêng chèn delay giữa các transaction để phủ small_gap/large_gap. |
...
|---|---|---|---|---|---|---|---|---|
| 1 | 2026-09-06 | uart_tx_test | tx_sequence (basic, `repeat(20)`) | 20 | 20 | 0 | **80.00%** (⚠️ vẫn là số trộn code+functional từ `$get_coverage()`, chưa tách riêng `cp_echoed_data`) | ✅ **Xác nhận PASS sạch** — log đầy đủ có `UVM Report Summary`: `UVM_ERROR:0`, `[uart_scoreboard]:20`, `[DRV]:20`, khớp đúng 20/20. Covergroup chỉ có 3 bin (`zero/all_ones/others[8]`), không có `alt1/alt2`. ⚠️ Nhưng phát hiện dấu hiệu bất thường về timing — xem ghi chú race condition bên dưới. |
| 2 | 2026-09-06 | uart_tx_test | tx_corner_data (đã chỉnh `repeat(20)`, không còn `repeat(100)` như thiết kế gốc; `constraint dist{...:/8}` đã fix) | 20 | ? | ? | 83.33% (10/12 bin) | ⚠️ **CHƯA CHỐT hoàn toàn** — 20 transaction khớp đúng với `repeat(20)` đã chỉnh (không phải lỗi thiếu log như nghi ngờ trước). Chỉ còn thiếu phần `UVM Report Summary` ở cuối log (không thấy dòng `UVM_ERROR`/`[uart_scoreboard]` count) để xác nhận UVM_ERROR=0 trước khi chốt chính thức. |
| 3 | 2026-09-09 | uart_tx_test | tx_corner_data (đổi lên `repeat(50)`) | 50 | 50 | 0 | **100% (12/12 bin)** | ✅ **Xác nhận PASS sạch, coverage đầy đủ** — log hoàn chỉnh: `UVM_ERROR:0`, `[uart_scoreboard]:50`, `[DRV]:50`, khớp đúng. Tăng từ `repeat(20)`→`repeat(50)` đủ để phủ nốt 2/8 bin `others[8]` còn thiếu ở run #2 — đúng nguyên tắc đã học: số lượng mẫu cần đủ lớn để phủ hết dải giá trị rộng. **Đây là dòng chính thức để tính tầng TX-corner đã đạt 100% functional coverage.** |
| 4 | 2026-09-09 | uart_tx_test | tx_corner_data (repeat(25), đã fix race condition: thêm wait(Tx_Ready==1'b0) trước wait(Tx_Ready==1'b1) trong tx_driver) | 25 | 25 | 0 | 91.67% (11/12 bin — thiếu 1 sub-bin others[8], do repeat(25) ít hơn run #3 (repeat(50), từng đạt 100%)) | ✅ XÁC NHẬN FIX RACE CONDITION THÀNH CÔNG — so sánh timestamp [DRV] giữa trước/sau fix: trước fix, 2-3 transaction đầu hoàn thành bất thường nhanh (30-210ns); sau fix, TOÀN BỘ 25 transaction có nhịp đều đặn ~87100ns/frame ngay từ transaction đầu tiên, không còn hiện tượng dính chùm. Xác nhận giả thuyết đúng: Tx_Ready cần vài chu kỳ mới thực sự tụt xuống 0 sau Tx_start, driver cũ không đợi bước này nên có thể đã trót coi 1 vài frame đầu là "xong" trong khi TX chưa kịp serialize — nay đã sửa triệt để. |
...
| 2 | 2026-09-15 | uart_test | echo_back_to_back_test (gửi 200 byte liên tục hoàn toàn không nghỉ; scoreboard kiểu "tập hợp" thay vì FIFO nghiêm ngặt; cp_echo_busy_overlap tính bằng ước lượng $time vì tầng Echo không có Tx_Ready để đọc trực tiếp) | 200 | 	126 match hợp lệ | 74 "torn data" (KHÔNG khớp bất kỳ giá trị đã gửi nào) + 2 byte biến mất hoàn toàn (198≠200) | **100%** (2/2 bin, xác nhận THẬT — trước đó từng là số giả do biến new_byte_during_tx_busy "mồ côi") | ✅ PASS đúng nghĩa (UVM_ERROR:0, đã đổi uvm_error→uvm_warning cho nhánh "torn data" vì đây là hiện tượng CHỦ ĐÍCH tìm ra, không phải lỗi bất ngờ). Kết quả định lượng quan trọng cho F8: trong điều kiện gửi dồn dập tuyệt đối, 37% (74/200) byte bị "torn data" (nghi do RX ghi đè Rx_Dout giữa lúc TX đang đọc dở — chưa xác nhận qua waveform), và 1% (2/200) byte biến mất hoàn toàn không được echo. Đây là bằng chứng định lượng đầu tiên cho giới hạn kiến trúc "không buffer" đã ghi từ đầu VPlan — không cần fix RTL (đúng bản chất thiết kế), chỉ cần ghi nhận rõ giới hạn này. ⚠️ Chưa xác nhận qua waveform giả thuyết "torn data" — vẫn là việc tồn đọng. |
| 3 | 2026-09-17 | uart_test | echo_stress_test (top_sequence có nghỉ giữa các frame, giống echo_random_test, tăng lên repeat(1000)) | 1000 | 	1000 | 0 | **100%**  | 	✅ PASS sạch hoàn toàn — 1000/1000 match, không có torn data nào (đúng như kỳ vọng vì driver này CÓ nghỉ giữa các frame, khác hẳn echo_back_to_back_test). CPU Time 19s cho 1000 transaction. ⚠️ Thử repeat(2000) bị "maximum runtime" — đã xác định đây là giới hạn thời gian thực thi của EDA Playground (platform limit), KHÔNG PHẢI bug logic như các lần hang trước đây (RX/TX reset-mid-frame): CPU time tăng gần tuyến tính theo số lượng (19s/1000 → dự kiến ~38s+/2000), rất có khả năng vượt ngưỡng cho phép của tài khoản. Không cần sửa code, chỉ cần dừng ở mức 1000 — đã đủ để xác nhận độ ổn định qua khối lượng lớn (mục đích chính của stress test), không cần cố đạt số lớn hơn vì coverage đã đạt 100% từ trước (run #3, repeat(500)). |
